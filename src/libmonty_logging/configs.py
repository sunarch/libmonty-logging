...
def get(identifier: Config, log_file_path: str = None) -> dict:
    ...
    if identifier is Config.STDOUT_AND_FILE_V1:
        if log_file_path is None:
            raise ValueError('Log file path required but not provided')

        formatter_1_name, formatter_1 = formatters.get(formatters.Formatter.TIME_AND_LEVEL_8_V1)
        chosen_formatters[formatter_1_name] = formatter_1
        handler_1_name, handler_1 = handlers.get(handlers.Handler.STDOUT_V1,
                                                 formatter_1_name)
        chosen_handlers[handler_1_name] = handler_1

        formatter_2_name, formatter_2 = formatters.get(formatters.Formatter.DATETIME_AND_LEVEL_8_V1)
        chosen_formatters[formatter_2_name] = formatter_2
        handler_2_name, handler_2 = handlers.get(handlers.Handler.FILE_V1,
                                                 formatter_2_name,
                                                 filename=log_file_path)
        chosen_handlers[handler_2_name] = handler_2
